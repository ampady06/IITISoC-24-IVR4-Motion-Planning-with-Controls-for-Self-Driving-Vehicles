...
n_states = 3
n_controls = 2
N = 50                                                                    # Prediction horizon(same as control horizon)
                                                                                                                
                      
error_allowed = 0.01

U_ref = np.array([0.5,0], dtype ='f')     				                    # Reference velocity and reference omega					                               		                                                                   

Q_x = 150                                                                  # gains to control error in x,y,theta during motion
Q_y = 150
Q_theta =  20                                                                                                                                            
...
def my_mainfunc():
    ...
    rospy.init_node('mpcpathTrack', anonymous=True)

    rospy.Subscriber('/odom', Odometry , odomfunc)    
   
    rospy.Subscriber('/my_path', Path, pathfunc)                                                                                                                                                                   
    
    instance = rospy.Publisher('/cmd_vel', Twist, queue_size=10)

    rate = rospy.Rate(10)
    rate.sleep() 
    while np.array_equal(path,np.array([])):
    ...
    for i in range(0,N):                                                                           
        ...
        print(path.shape, close_index, N,i)                                                    
